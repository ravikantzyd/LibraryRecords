...
{
    public partial class LIB_EXTENDED_DAY_SIGNATURE_VIEW_FORM : Form
    {
        float PointX;
        float PointY;
        float LastX;
        ...
        public LIB_EXTENDED_DAY_SIGNATURE_VIEW_FORM()
        {
            InitializeComponent();
        }

        private async void LIB_EXTENDED_DAY_SIGNATURE_VIEW_FORM_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (records != null)
                    {
                        string SignaturePoints = records[0].DExtendedSignature;
                        ...
                        if (SignaturePoints != null)
                        {
                            for (int i = 0; i < SignaturePoints.Split('/').Length - 1; i++)
                            {
                                ...
                                lib_extended_day_sign_borrow_signature_panel_Paint(this, null);
                            }
                        }
                        else
                        {
                            MessageBox.Show("This book haven't been extended day.");
                            this.Close();
                        }
                    }
                }
        ...
        private void lib_extended_day_sign_borrow_signature_panel_Paint(object sender, PaintEventArgs e)
        {
            Graphics G = lib_extended_day_sign_borrow_signature_panel.CreateGraphics();
            G.DrawLine(Pens.Black, PointX, PointY, LastX, LastY);
            LastX = PointX;
            LastY = PointY;
        }

        private void lib_extended_day_sign_title_bar_panel_MouseDown(object sender, MouseEventArgs e)
        {
            LIB_FORM_ANIMATION.Form_Drag(this, e);
        }

        private void lib_extended_day_sign_close_btn_Click(object sender, EventArgs e)
        {
            Close();
        }
    }
